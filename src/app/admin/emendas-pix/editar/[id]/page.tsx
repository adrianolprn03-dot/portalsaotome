...
export default function EditarEmendaPixPage() {
    ...
    const loadData = useCallback(async () => {
        try {
            const res = await fetch(`/api/admin/emendas-pix/${params.id}`);
            if (res.ok) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                toast.success("Emenda PIX atualizada!");
    ...
    if (fetching) return <div className="flex justify-center p-20"><FaSpinner className="animate-spin text-4xl text-emerald-500" /></div>;
        ...
        <div className="max-w-5xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                ...
                <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                   <FaHandHoldingDollar className="text-emerald-500" /> Editar Emenda PIX
                </h1>
            </div>
