    ...
    const fetchServidores = async () => {
        ...
        try {
            const query = new URLSearchParams({
                ano,
                mes,
                vinculo: "terceirizado", // fixo para terceirizados
        ...
        <div className="min-h-screen bg-[#f8fafc] font-['Montserrat',sans-serif]">
            ...
            <div className="max-w-7xl mx-auto px-6 py-12 -mt-10 relative z-30">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
                    <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 border-l-4 border-l-orange-500 hover:shadow-xl hover:shadow-orange-500/5 transition-all group">
                        <div className="flex justify-between items-start mb-3">
                            <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Custo Bruto Mensal</div>
                        ...
                        <div className="mt-2 text-[9px] font-bold text-orange-500 uppercase tracking-tighter">Período Selecionado</div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 border-l-4 border-l-emerald-500 hover:shadow-xl hover:shadow-emerald-500/5 transition-all group">
                        <div className="flex justify-between items-start mb-3">
                            <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Custo Líquido Mensal</div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 border-l-4 border-l-orange-500 hover:shadow-xl hover:shadow-orange-500/5 transition-all group">
                        <div className="flex justify-between items-start mb-3">
                            <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Nº de Terceirizados</div>
                            <FaUserTie className="text-orange-100 group-hover:text-orange-500 transition-colors" size={20} />
                        </div>
                        <div className="text-xl font-black text-orange-600">{loading ? "..." : filtrados.length}</div>
                        <div className="mt-2 text-[9px] font-bold text-orange-500 uppercase tracking-tighter">Postos Ativos</div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
                    <div className="p-5 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                       <h3 className="text-[10px] font-black text-gray-800 uppercase tracking-widest flex items-center gap-2">
                           <span className="w-1.5 h-4 bg-orange-500 rounded-full"></span> Relação de Terceirizados
                       </h3>
                    ...
                    <div className="overflow-x-auto min-h-[300px] relative">
                        {loading && (
                            <div className="absolute inset-0 bg-white/70 flex flex-col items-center justify-center z-10 backdrop-blur-sm">
                                <FaSpinner className="animate-spin text-orange-500 text-3xl mb-3" />
                                <div className="text-[9px] font-black text-orange-600 uppercase tracking-[0.2em]">Consultando Dados...</div>
                            </div>
                        )}
                        <table className="w-full text-left" aria-label="Tabela de terceirizados">
                            <thead>
                                <tr className="bg-gray-50/50 text-[9px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">
                                    <th className="px-6 py-4">Nome / Posto</th>
                                ...
                                {filtrados.length === 0 && !loading ? (
                                    <tr>
                                        <td colSpan={4} className="px-6 py-20 text-center text-gray-400 font-bold uppercase text-[10px] tracking-widest italic">
                                            Nenhum registro localizado para o período.
                                        </td>
                                    </tr>
                                ) : (
                                    filtrados.map((s: Servidor) => (
                                        <tr key={s.id} className="hover:bg-orange-50/30 transition-all group">
                                            <td className="px-6 py-4">
                                                <div className="font-black text-gray-800 text-[11px] group-hover:text-orange-700 transition-colors uppercase tracking-tight">{s.nome}</div>
                            ...
                            {!loading && servidores.length > 0 && (
                                <tfoot>
                                    <tr className="bg-gray-900 border-t-2 border-gray-800 text-white">
                                        ...
                                        <td className="px-6 py-4 text-right font-black text-base text-orange-400">{fmt(totalBruto)}</td>
